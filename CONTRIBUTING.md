...
You should also read our [coding guide](https://github.com/darshanman40/influxdb/blob/master/CODING_GUIDELINES.md), to understand better how to write code for InfluxDB.
...
```bash
    ...
    go get github.com/darshanman40/influxdb
```
...
```bash
    export GOPATH=$HOME/gocodez
    mkdir -p $GOPATH/src/github.com/darshanman40/influxdb
    cd $GOPATH/src/github.com/darshanman40/influxdb
    git clone git@github.com:<username>/influxdb
```

Retaining the directory structure `$GOPATH/src/github.com/darshanman40/influxdb` is necessary so that Go imports work correctly.
...
-----
InfluxDB uses CircleCI for continuous integration testing. To see how the code is built and tested, check out [this file](https://github.com/darshanman40/influxdb/blob/master/circle-test.sh). It closely follows the build and test process outlined above. You can see the exact version of Go InfluxDB uses for testing by consulting that file.
